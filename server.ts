...
// API Tóm tắt nội dung Chỉ đạo / Đầu việc thành các gạch đầu dòng súc tích
app.post("/api/ai/summarize-task-bullets", async (req, res) => {
  try {
    const { text, type = "TASK" } = req.body;
    if (!text || typeof text !== "string" || !text.trim()) {
      return res.status(400).json({ success: false, error: "Thiếu nội dung văn bản cần tóm tắt" });
    }

    const ai = getAIInstance();
    const prompt = `
Bạn là Trợ lý AI Quản Lý Sản Xuất & Chất Lượng 4M1E1I.
Nhiệm vụ của bạn: Tóm tắt nội dung ${type === "DIRECTIVE" ? "CHỈ ĐẠO" : "ĐẦU VIỆC / NHIỆM VỤ"} dưới đây thành các GẠCH ĐẦU DÒNG cực kỳ súc tích, rõ ràng, dễ nhớ và hành động được ngay.
...
Yêu cầu tóm tắt:
1. Bóc tách các ý thành từng gạch đầu dòng rõ ràng, nhóm theo chủ đề hoặc bước thực hiện nếu có nhiều mục (ví dụ: Nguyên nhân, Giải pháp, Tiêu chuẩn, Hành động cần làm).
2. Viết ngắn gọn, trực diện, giữ đúng số liệu/thông số kỹ thuật và căn cứ tiêu chuẩn (như công suất W, % tăng giảm, tiêu chuẩn TCVN, quyết định...).
3. Trả về định dạng JSON DUY NHẤT theo cấu trúc:
{
  "summaryTitle": "Tiêu đề tóm lược cực ngắn (dưới 12 từ)",
  "sections": [
    {
      "heading": "Tên nhóm ý (ví dụ: Quy định nhãn năng lượng / Nguyên nhân kỹ thuật / Giải pháp hướng dẫn...)",
      "bullets": [
        "Gạch đầu dòng 1 rõ ràng, có điểm nhấn",
        "Gạch đầu dòng 2..."
      ]
    }
  ],
  "actionItem": "Hành động then chốt cần thực hiện ngay (nếu có)"
}
`;
    ...
    try {
      ...
      console.warn("Could not parse JSON from task summarize output");
    }
