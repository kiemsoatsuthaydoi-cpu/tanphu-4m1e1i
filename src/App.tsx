...
import { FestiveTopBanner } from "./components/FestiveTopBanner";
import { generateNotifications } from "./utils/notificationHelper";
import { getCategoryFallbackImage } from "./utils/imageProcessor";
import { extractTaggedUserIds, getDefaultMembersForReport } from "./utils/userResolver";
import {
  initialUsers,
  initialReports,
  initialCompanies,
...
const sanitizeAndMigrateDepartments = (rawDepts: Department[]): Department[] => {
  ...
  let list = rawDepts.map(d => {
    ...
    const initialMatch = initialDepartments.find(initD => initD.id === d.id || initD.name === d.name);
    const shortName = d.shortName || initialMatch?.shortName;
    return { ...d, branchId: bId, name, shortName };
  });
  ...
  if (!bbmQCExist) {
    list.push({ id: "bbm-1", name: "Phòng Quản Lý Chất Lượng (DNP-BBM)", shortName: "QLCL", branchId: "DNP-BBM" });
  }
  const bbmKTExist = list.some(d => d.branchId === "DNP-BBM" && d.name.includes("Tài chính Kế toán"));
  if (!bbmKTExist) {
    list.push({ id: "bbm-2", name: "Phòng Tài chính Kế toán (DNP-BBM)", shortName: "TCKT", branchId: "DNP-BBM" });
  }

  const bbcQCExist = list.some(d => d.branchId === "DNP-BBC" && d.name.includes("Quản Lý Chất Lượng"));
  if (!bbcQCExist) {
    list.push({ id: "bbc-1", name: "Phòng Quản Lý Chất Lượng (DNP-BBC)", shortName: "QLCL", branchId: "DNP-BBC" });
  }
  const bbcKTExist = list.some(d => d.branchId === "DNP-BBC" && d.name.includes("Tài chính Kế toán"));
  if (!bbcKTExist) {
    list.push({ id: "bbc-2", name: "Phòng Tài chính Kế toán (DNP-BBC)", shortName: "TCKT", branchId: "DNP-BBC" });
  }
...
export default function App() {
  ...
  const [festiveBannerConfig, setFestiveBannerConfig] = useState<FestiveBannerConfig | null>(() => {
    const saved = safeGetItem("4m1e1i_festive_banner_config");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
  ...
  useEffect(() => {
    if (festiveBannerConfig) {
      safeSetItem("4m1e1i_festive_banner_config", JSON.stringify(festiveBannerConfig));
    }
  }, [festiveBannerConfig]);
  ...
  const handleUpdateFestiveBannerConfig = useCallback((newConfig: FestiveBannerConfig) => {
    setFestiveBannerConfig(newConfig);
    safeSetItem("4m1e1i_festive_banner_config", JSON.stringify(newConfig));
    if (dbConnected) {
      saveDocument("config", "festive_banner", newConfig).catch(console.error);
    }
    showToast("Đã lưu và cập nhật cấu hình Banner Lễ Hội thành công!", "success");
  }, [dbConnected]);
  ...
  const syncFromDb = async (isManual = false) => {
    ...
    try {
      ...
      const remoteFestiveBanner = fConfigs.find((c: any) => c.id === "festive_banner");
      if (remoteFestiveBanner) {
        setFestiveBannerConfig(remoteFestiveBanner);
      }
  ...
  const handleAddForumTopic = (
    ...
    // Extract any tagged users in topic title or description
    const tagged = extractTaggedUserIds(`${title} ${description}`, users);
    const initialInvited = invitedUserIds ? [...invitedUserIds] : [];
    
    // If topic is created for a Report (Bản tin KPH / chuyên đề), automatically ensure creator, uploader, and tagged members are included
    if (reportId) {
      const rep = reports.find((r) => r.id === reportId || r.reportCode === reportId);
      if (rep) {
        const repDefaults = getDefaultMembersForReport(rep, users, currentUser);
        repDefaults.memberIds.forEach((mId) => {
          if (!initialInvited.includes(mId)) {
            initialInvited.push(mId);
          }
        });
      }
    }

    tagged.userIds.forEach((uId) => {
      if (!initialInvited.includes(uId)) {
        initialInvited.push(uId);
      }
    });
    ...
    const newTopic: ForumTopic = {
      ...
      isPinned: false,
      reportId,
      invitedUserIds: initialInvited.length > 0 ? initialInvited : undefined
    };
    ...
    if (tagged.userNames.length > 0) {
      showToast(`Đã tạo chủ đề và tự động thêm ${tagged.userNames.join(", ")} vào nhóm! 👥`, "success");
    } else {
  ...
  const handleAddForumReply = (
    ...
    // Auto-detect tagged or assigned individual and add them into the topic group
    const tagged = extractTaggedUserIds(
      `${message} ${extraData?.actionData?.note || ""}`,
      users,
      extraData?.actionData
        ? {
            id: extraData.actionData.assignedToId,
            fullName: extraData.actionData.assignedToName,
          }
        : undefined
    );

    if (tagged.userIds.length > 0) {
      setTopics((prevTopics) => {
        let addedNames: string[] = [];
        const nextTopics = prevTopics.map((t) => {
          if (t.id !== topicId) return t;
          const currentInvited = t.invitedUserIds ? [...t.invitedUserIds] : [];
          let hasChange = false;

          tagged.userIds.forEach((uId, idx) => {
            if (!currentInvited.includes(uId)) {
              currentInvited.push(uId);
              hasChange = true;
              const name = tagged.userNames[idx];
          ...
          if (hasChange) {
            const updatedTopic: ForumTopic = {
              ...t,
              invitedUserIds: currentInvited
            };
            if (dbConnected) {
              saveDocument(COLLECTIONS.TOPICS, updatedTopic.id, updatedTopic).catch((err) => {
                console.error("Lỗi khi cập nhật thành viên vào chủ đề:", err);
              });
            }
            return updatedTopic;
          }
        ...
        if (addedNames.length > 0) {
          showToast(`Đã tự động thêm ${addedNames.join(", ")} vào nhóm thảo luận! 👥`, "info");
        }
  ...
  const handleEditForumReply = (
    ...
    // Check if edited message or actionData contains tagged/assigned users
    const textToCheck = typeof updatedData === "string" ? updatedData : (updatedData.message || "");
    const actionDataToCheck = typeof updatedData === "object" ? updatedData.actionData : undefined;
    if (targetTopicId && (textToCheck || actionDataToCheck)) {
      const tagged = extractTaggedUserIds(
        `${textToCheck} ${actionDataToCheck?.note || ""}`,
        users,
        actionDataToCheck
          ? {
              id: actionDataToCheck.assignedToId,
              fullName: actionDataToCheck.assignedToName,
            }
          : undefined
      );
  ...
  const handleEditForumTopic = (
    ...
    // Extract any tagged users in updated title/description
    const tagged = extractTaggedUserIds(`${title} ${description}`, users);
    ...
    setTopics((prev) => prev.map((t) => {
      ...
      const updated = {
        ...
        description,
        category,
        invitedUserIds: currentInvited.length > 0 ? currentInvited : undefined
      };
      if (dbConnected) {
        saveDocument(COLLECTIONS.TOPICS, updated.id, updated).catch((err) => {
  ...
  const handleToggleLikeChatMessage = (chatId: string) => {
    if (!currentUser) {
      showToast("Vui lòng đăng nhập để thực hiện chức năng này!", "warning");
      return;
    }
    const userIdentifier = currentUser.fullName ? formatNameCapitalized(currentUser.fullName) : (currentUser.id || "Người dùng");
      ...
      prev.map((c) => {
        if (c.id === chatId) {
          ...
          const newLikedBy = hasLiked
            ? likedBy.filter((u) => u !== userIdentifier)
          ...
          if (dbConnected) {
            saveDocument(COLLECTIONS.CHATS, chatId, updatedChat).catch((err) => {
              console.error("Lỗi khi lưu thả tim tin nhắn lên Firestore:", err);
            });
          }
          return updatedChat;
        }
  ...
  return (
    <div className="h-screen max-h-screen bg-[#F0F2F5] flex flex-col relative overflow-hidden font-sans">
      <FestiveTopBanner
        config={festiveBannerConfig}
        isAdmin={currentUser?.role === UserRole.ADMIN}
      />
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden h-full">
        <div className="hidden lg:flex flex-1 min-w-0 flex-col dashboard-desktop-wrapper h-full overflow-hidden">
          <DashboardDesktop
            currentUser={currentUser}
            users={users}
            reports={reports}
